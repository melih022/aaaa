...
async def executor(client, message):
    if len(message.command) < 2:
        return await edit_or_reply(
            message, text="__MİSS Bana yürütmem için bir komut ver.__"
        )
    try:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="⏳",
                        callback_data=f"Çalışma süresi {round(t2-t1, 3)} Seconds",
                    ),
                    InlineKeyboardButton(
                        text="🗑",
                        callback_data=f"abc'yi zorla|{message.from_user.id}",
                    ),
                ]
            ]
        )
