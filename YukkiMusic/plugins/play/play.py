...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await Telegram.download(_, message, mystic, file_path):
            ...
            try:
                ...
                err = (
                    ...
                    else f"{_['general_3'].format(ex_type)}" + f"\n\n`{e}`"
                )
                try:
                    from YukkiMusic.plugins.devs.diagnostics import LAST_ERRORS
                    import traceback as _tb
                    LAST_ERRORS.append(f"play.stream() -> {type(e).__name__}: {e}\n" + _tb.format_exc())
        ...
        if not await is_video_allowed(message.chat.id):
            return await mystic.edit_text(f"{_['play_3']}\n\n`{type(_e).__name__}: {_e}`"[:1000] if "_e" in dir() else _["play_3"])
        ...
        if await YouTube.exists(url):
            if "playlist" in url:
                try:
                    ...
                    _record_play_error(f"play_3 ({type(e).__name__}: {e})")
                    return await mystic.edit_text(f"{_['play_3']}\n\n`{type(e).__name__}: {e}`"[:1000])
                streamtype = "playlist"
                plist_type = "yt"
                if "&" in url:
            ...
            if "track" in url:
                try:
                    ...
                    _record_play_error(f"play_3 ({_e!r})")
                    return await mystic.edit_text(f"{_['play_3']}\n\n`{type(_e).__name__}: {_e}`"[:1000] if "_e" in dir() else _["play_3"])
                streamtype = "youtube"
                img = details["thumb"]
                cap = _["play_11"].format(
            ...
            if "album" in url:
                ...
                return await mystic.edit_text(_["play_16"] + "\n\n💡 yt-dlp veya kaynak platformdan sonuç dönmedi. /testytdlp ile test edin.")
            ...
            try:
                ...
                return await mystic.edit_text(
                    f"{_['general_3'].format(type(e).__name__)}" + f"\n\n`{e}`"
                )
    ...
    if str(playmode) == "Direct":
        ...
        if plist_type:
            ...
            await mystic.delete()
            await _safe_reply_photo(message, photo=img,
                caption=cap,
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            return await play_logs(
            ...
            if slider:
                ...
                await mystic.delete()
                await _safe_reply_photo(message, photo=details["thumb"],
