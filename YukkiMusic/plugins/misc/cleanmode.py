        ...
        try:
            ...
            for chat_id in served_chats:
                if chat_id not in adminlist:
                    ...
                    admins = []
                    async for _m in app.get_chat_members(
                        chat_id, filter=ChatMembersFilter.ADMINISTRATORS
                    ):
                        admins.append(_m)
                    for user in admins:
                        if (getattr(user.privileges, 'can_manage_video_chats', False) if getattr(user, 'privileges', None) else getattr(user, 'can_manage_video_chats', False)):
